...
#define date_and_time                                                          \
  17 // number of characters within the format of MM//DD/YYYY 00:00 (including
     // spaces and '/')
#define max_event 50 // max number of event for one day

const int start_day = 15, start_month = 10,
          start_year = 1582; // Calendar starts on October 15, 1582

const int monthDays[] = {31, 28, 31, 30, 31, 30,
...
// Calendar Visuals functions
void calendarChoice(); // where decision on type of calendar occurs (month or
                       // year) finds events for specific month and year
void date_read(char relevantEvents[max_event][char_lim], char *fileName,
               int target_month, int target_day, int target_year);

// Event functions
int returnUser(
    const char *fileName); // searches users disk for existing event file
int addEvent(
    char *fileName); // creates user specific event file for event storage
int modifyEvent(
    const char *fileName); // changes saved event based on event date and time
...
int main() {
  ...
  fileName = malloc(sizeof(char) *
                    (strlen(username) + strlen(txt) + 1)); // + 1 for '\0'
  if (fileName == NULL) {
    printf("failed to make pointer");
    return -1;
  }
...
int returnUser(const char *fileName) {
  ...
  if (file != NULL) {
    printf("Welcome back: %.*s! You have saved event \n", strlen(fileName) - 4,
           fileName); // prints only the user name (4  = .txt)
    fclose(file);
    return TRUE;
  } else {
    printf("Welcome new user: %.*s! You have no saved event \n",
...
void calendarChoice() {
  ...
  int addEvent(char *fileName) {
    ...
    do {
      ...
      if (toupper(input) == 'Y') {
        printf(
            "Input the date (MM/DD/YYYY), the time, following a 24-hour clock "
            "(00:00), of the event and the event (all separated by space): \n");
        printf("Example: 12/34/5678 23:59 'Lorem Ipsum' \n");
        ...
        if (fprintf(ptr, "%s\n", event) <
            0) { // if no lines printed onto file, quit
          perror("Error writing to the file\n");
          fclose(ptr);
          return 1;
        }
        ...
        if (fflush(ptr) !=
            0) { // ensures event is printed onto file, quits if fails
          perror("Error flushing the file\n");
          fclose(ptr);
          return 1;
        }
  ...
  int modifyEvent(const char *fileName) {
    ...
    printf("Input the date (MM/DD/YYYY), the time, following a 24-hour clock "
           "(00:00), of the event being changed (separated by a space):\n");
    printf(
        "And press enter twice: once to save and one extra time to submit\n");

    fgets(lineEdit, sizeof(lineEdit), stdin);
    clearInputBuffer();

    while (fgets(event, sizeof(event), ptr) !=
           NULL) { // Keeps scanning until EOF reached
      if (strncmp(event, lineEdit, strlen(lineEdit)) == 0) {
        printf("Enter the new event: ");
        fgets(lineEdit, sizeof(char) * char_lim, stdin);
        clearInputBuffer();
  ...
  int zeller() {
    ...
    // returns a value between 0-6 -> 0 = Saturday, 1 = Sunday ...
    int zellerDay = (start_day + (13 * (start_month + 1) / 5) + K + (K / 4) +
                     (J / 4) - 2 * J) %
                    week;

    // adjusts zeller values to traditional calendar values sunday = 0, monday =
    // 1, ...
    return (zellerDay + 6 + week) % 7; // + week ensures return is positive
  }
  ...
  int numberOfDaysApart(int target_year, int target_month) {
    ...
    int daysLeftStart =
        full_year -
        totalDaysIntoYear(start_month,
                          start_day); // subtracts remaining days in the year
    ...
    if (target_month == 0)
      daysIntoCurrent =
          totalDaysIntoYear(target_month, 0); // day = 0 b/c start of month
    else
      daysIntoCurrent =
          totalDaysIntoYear(target_month, 1); // day = 0 b/c start of month
